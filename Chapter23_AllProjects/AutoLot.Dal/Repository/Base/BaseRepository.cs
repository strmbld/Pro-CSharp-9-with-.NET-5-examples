...
namespace AutoLot.Dal.Repository.Base
{
    public abstract class BaseRepository<T> : IRepository<T> where T : BaseEntity, new()
    {
        ...
        public virtual T? FindOneByIdAsNoTracking(int id) => Table.AsNoTrackingWithIdentityResolution().FirstOrDefault(x => x.Id == id);
